...
const buildServicePath = path.join(__dirname, 'node_modules/@insidethesim/remix-dev/dist/cli/build-service.js');
...
async function main() {
  try {
    ...
    if (result.sdkIntegration) {
      const status = result.sdkIntegration.integrated ? 'Integrated' : 'Not integrated';
      console.log(`SDK: ${status} (${result.sdkIntegration.passedChecks}/${result.sdkIntegration.totalChecks} checks)`);
    }
